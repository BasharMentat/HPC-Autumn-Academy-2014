PROGRAM CubeRoot
    USE double
    IMPLICIT NONE
    COMPLEX(KIND=DP) :: target, result
    INTEGER :: i, k
mainloop: DO
        PRINT *, 'Type in a COMPLEX number'
...
CONTAINS
    ...
    FUNCTION Newton (source)
        COMPLEX(KIND=DP) :: Newton
        COMPLEX(KIND=DP), INTENT(IN) :: source
        COMPLEX(KIND=DP) :: current
        COMPLEX(KIND=DP), DIMENSION(5) :: previous
!
...
FUNCTION value (arg, targ)
    USE double
    IMPLICIT NONE
    COMPLEX(KIND=DP) :: value, arg, targ
    value = arg**3-targ
END FUNCTION value
...
FUNCTION derivative (arg)
    USE double
    IMPLICIT NONE
    COMPLEX(KIND=DP) :: derivative, arg
    derivative = 3*arg**2
